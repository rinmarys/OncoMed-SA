...
function SolicitacaoConsulta() {
    ...
    const fetch_marcarConsulta = async () => {
        try {
            ...
            setConsultaId(id_marcarConsulta);
        } catch (error) {
    ...
    return (
        <form onSubmit={'submit'}>
            <div className="tudo-solicitacao">
                                ...
                                <div className="designar-botoes-solicitacao">
                                    <select id="escolha-medico" className="designar-profissional-input">
                                        ...
                                        {lista_de_medicos.map((medico) => (
                                            <option key={medico.id} value={medico.id_consulta}>
                                                {medico.nome}
                                            </option>
                                        ))}
                                    </select>
                                    <div className="botoes-solicitacao">
                                        ...
                                        <button className="botao-cancela-solicitacao" onClick={() => handleOpenPopup(consulta.id_consulta)} type="button">
                                        {/* <button className="botao-cancela-solicitacao" onClick={() => alert(consulta.id_consulta)} type="button"> */}
                                            CANCELAR
                                        </button>
                ...
                {isOpen && (
                    <CancelarConsultaAdmin onClose={handleClosePopup} consultaId={consultaId} fetch_marcarConsulta={fetch_marcarConsulta} />
                )}
            </div>
        </form>
    );
